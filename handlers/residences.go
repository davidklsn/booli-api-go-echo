...
func DeleteUserResidence(id string, residence map[string]any) (types.UserData, error) {
	var userData types.UserData
	if err := constants.DB.First(&userData, "user_id = ?", id).Error; err != nil {
		return types.UserData{}, err
	}
	...
	errors := helpers.DeleteResidence(&existingResidences, residence)
...
func GetSelectedResidence(id string) (map[string]any, error) {
	var UserData types.UserData
	var residences []map[string]any
	if err := constants.DB.First(&UserData, "user_id = ?", id).Select("residences").Error; err != nil {
		return make(map[string]any), err
	...
	for _, r := range residences {
		if val, ok := r["selecteResidence"]; ok && val == true {
			return r, nil
		}
	}
